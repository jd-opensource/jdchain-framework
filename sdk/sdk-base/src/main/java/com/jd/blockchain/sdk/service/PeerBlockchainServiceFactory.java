...
public class PeerBlockchainServiceFactory implements BlockchainServiceFactory, Closeable {
     ...
     * @param manageSslSecurity    提供对网关接入认证的节点的管理服务TLS证书相关信息； <br>
     * @param consensusSslSecurity 提供对网关接入认证的节点的公式服务TLS证书相关信息； <br>
     * @param credentialProvider   共识客户端在接入认证过程中使用的凭证的来源
     * @param clientManager        账本对应的共识客户端管理
     * @return 区块链服务工厂实例
     */
    public static PeerBlockchainServiceFactory connect(AsymmetricKeypair gatewayKey, NetworkAddress peerAddr, SSLSecurity manageSslSecurity,
                                                       SSLSecurity consensusSslSecurity, SessionCredentialProvider credentialProvider, ConsensusClientManager clientManager) {
        // 认证，返回账本列表及配置相关信息
        PeerAuthenticator authenticator = new PeerAuthenticator(peerAddr, manageSslSecurity, gatewayKey, credentialProvider);
        GatewayAuthResponse gatewayAuthResponse = authenticator.request();

        return create(gatewayKey, peerAddr, manageSslSecurity, consensusSslSecurity, gatewayAuthResponse.getLedgers(), credentialProvider, clientManager);
    }
    ...
    public static PeerBlockchainServiceFactory connect(AsymmetricKeypair gatewayKey, NetworkAddress peerAddr,
        ...
        return connect(gatewayKey, peerAddr, new SSLSecurity(), new SSLSecurity(), credentialProvider, clientManager);
    }
