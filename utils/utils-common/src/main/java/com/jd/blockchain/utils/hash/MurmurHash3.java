...
public final class MurmurHash3 {
	 ...
	 * @param out    out
	 */
	public static void murmurhash3_x64_128(byte[] key, int offset, int len, int seed, byte[] hash) {
		murmurhash3_x64_128(key, offset, len, seed, hash, 0);
	}
	 ...
	 * @param out    out
	 */
	public static void murmurhash3_x64_128(byte[] key, int offset, int len, int seed, byte[] hash, int hashOffset) {
		long[] result = new long[2];
		murmurhash3_x64_128(key, offset, len, seed, result);

		BytesUtils.toBytes(result[0], hash, hashOffset);
		BytesUtils.toBytes(result[1], hash, hashOffset + 64);
	 ...
	 * @param out    out
	 */
	public static byte[] murmurhash3_x64_128(byte[] key, int offset, int len, int seed) {
		long[] result = new long[2];
		murmurhash3_x64_128(key, offset, len, seed, result);

		byte[] hash = new byte[128];
		BytesUtils.toBytes(result[0], hash, 0);
		BytesUtils.toBytes(result[1], hash, 64);
	 ...
	 * @return The first 64 bits of hash;
	 */
	public static long murmurhash3_x64_64_1(byte[] key, int offset, int len, int seed) {
		// The original algorithm does have a 32 bit unsigned seed.
		// We have to mask to match the behavior of the unsigned types and prevent sign
		// extension.
		long h1 = seed & 0x00000000FFFFFFFFL;
	...
	/**
	 * Compute the MurmurHash3_x64_128 hash, and return the second 64 bits.
	 * 
	 * @param key    key
	 * @param offset offset
	 * @param len    len
	 ...
	 * @param out    out
	 */
	public static long murmurhash3_x64_64_2(byte[] key, int offset, int len, int seed) {
		// The original algorithm does have a 32 bit unsigned seed.
		// We have to mask to match the behavior of the unsigned types and prevent sign
		// extension.
		long h1 = seed & 0x00000000FFFFFFFFL;
