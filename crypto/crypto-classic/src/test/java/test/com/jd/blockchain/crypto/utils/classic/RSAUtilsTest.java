...
public class RSAUtilsTest {
	...
	private void keyPairTest(AsymmetricCipherKeyPair keyPair) {
		...
		byte[] pubKeyBytesConverted_RawKey = RSAUtils
				.pubKey2Bytes_RawKey(RSAUtils.bytes2PubKey_RawKey(pubKeyBytes_RawKey));
		...
		byte[] privKeyBytesConverted_RawKey = RSAUtils
				.privKey2Bytes_RawKey(RSAUtils.bytes2PrivKey_RawKey(privKeyBytes_RawKey));
		...
		byte[] pubKeyBytesConverted_PKCS1 = RSAUtils.pubKey2Bytes_PKCS1(RSAUtils.bytes2PubKey_PKCS1(pubKeyBytes_PKCS1));
		...
		byte[] privKeyBytesConverted_PKCS1 = RSAUtils
				.privKey2Bytes_PKCS1(RSAUtils.bytes2PrivKey_PKCS1(privKeyBytes_PKCS1));
		...
		byte[] pubKeyBytesConverted_PKCS8 = RSAUtils.pubKey2Bytes_PKCS8(RSAUtils.bytes2PubKey_PKCS8(pubKeyBytes_PKCS8));
		...
		byte[] privKeyBytesConverted_PKCS8 = RSAUtils
				.privKey2Bytes_PKCS8(RSAUtils.bytes2PrivKey_PKCS8(privKeyBytes_PKCS8));
	...
	public void test() {
		...
		plaintextFromPrivKey = RSAUtils.decrypt(ciphertext, privKey);
		plaintextFromPrivKeyBytes = RSAUtils.decrypt(ciphertext, privKeyBytes);
		...
		int cipherSize = RSAUtils.encrypt(data, pubKey, out);
		ciphertext = out.toByteArray();
		assertEquals(cipherSize, ciphertext.length);

		out = new ByteArrayOutputStream();
		int plainSize = RSAUtils.decrypt(ciphertext, privKeyBytes, out);
		plaintextFromPrivKeyBytes = out.toByteArray();
		
		assertEquals(plainSize, plaintextFromPrivKeyBytes.length);
		assertArrayEquals(data, plaintextFromPrivKeyBytes);
	}
...
//    @Test
	public void encryptionConsistencyTest() {
		...
		for (int r = 0; r < 5; r++) {
			...
			long elapsedTS = System.currentTimeMillis() - startTS;
			System.out.println(String.format("BouncyCastle-based RSA Encrypting Count=%s; Elapsed Times=%s; TPS=%.2f",
					count, elapsedTS, (count * 1000.00D) / elapsedTS));
		}
		...
		for (int r = 0; r < 5; r++) {
			...
			long elapsedTS = System.currentTimeMillis() - startTS;
			System.out.println(String.format("BouncyCastle-based RSA Decrypting Count=%s; Elapsed Times=%s; TPS=%.2f",
					count, elapsedTS, (count * 1000.00D) / elapsedTS));
		}
		...
		for (int r = 0; r < 5; r++) {
			...
			long elapsedTS = System.currentTimeMillis() - startTS;
			System.out.println(String.format("JDK-based RSA Encrypting Count=%s; Elapsed Times=%s; TPS=%.2f", count,
					elapsedTS, (count * 1000.00D) / elapsedTS));
		}
		...
		for (int r = 0; r < 5; r++) {
			...
			long elapsedTS = System.currentTimeMillis() - startTS;
			System.out.println(String.format("JDK-based RSA Decrypting Count=%s; Elapsed Times=%s; TPS=%.2f", count,
					elapsedTS, (count * 1000.00D) / elapsedTS));
		}
...
//    @Test
	public void signatureConsistencyTest() {
		...
		for (int r = 0; r < 5; r++) {
			...
			long elapsedTS = System.currentTimeMillis() - startTS;
			System.out.println(String.format("BouncyCastle-based RSA Signing Count=%s; Elapsed Times=%s; TPS=%.2f",
					count, elapsedTS, (count * 1000.00D) / elapsedTS));
		}
		...
		for (int r = 0; r < 5; r++) {
			...
			long elapsedTS = System.currentTimeMillis() - startTS;
			System.out.println(String.format("BouncyCastle-based RSA Verifying Count=%s; Elapsed Times=%s; TPS=%.2f",
					count, elapsedTS, (count * 1000.00D) / elapsedTS));
		}
		...
		for (int r = 0; r < 5; r++) {
			...
			long elapsedTS = System.currentTimeMillis() - startTS;
			System.out.println(String.format("JDK-based RSA Signing Count=%s; Elapsed Times=%s; TPS=%.2f", count,
					elapsedTS, (count * 1000.00D) / elapsedTS));
		}
		...
		for (int r = 0; r < 5; r++) {
			...
			long elapsedTS = System.currentTimeMillis() - startTS;
			System.out.println(String.format("JDK-based RSA Verifying Count=%s; Elapsed Times=%s; TPS=%.2f", count,
					elapsedTS, (count * 1000.00D) / elapsedTS));
		}
